 ...
 * All rights reserved.
 *
 * $Header: PTPDnldRecv.h 1.1 2007/12/06 11:41:20PST Zheng Miao (zmiao) Exp  $
 * $Revision: 1.1 $
 *
 */
...
{
public:
	void SetDelayedWriting(int8 delayed);
	void TickTime(int seconds);
	enum { BIG_DNLD_PACKET_SIZE = 1024, SMALL_DNLD_PACKET_SIZE = 64 };
	int ReceivePacket(CMemBlock *pBlock);
	CPTPDnldRecv(CStorage *pStorage, uint32 maxSize, uint16 cmd,  
		CPTPLink *pLink, uint8 outPort, uint8 bCheckHeader = 0);
	...
	typedef struct {
		...
		uint32 total;
		uint32 offset;
		uint8 content[BIG_DNLD_PACKET_SIZE];
	} SDnldBigCmd;
	typedef struct {
		uint16 cmd; 
		uint32 handle;
		uint32 total;
		uint32 offset;
		uint8 content[SMALL_DNLD_PACKET_SIZE];
	} SDnldSmallCmd;
	typedef struct {
		uint16 cmd;
		uint32 handle;
...
private:
	...
	uint8 m_outPort;
	int m_timer;
	uint8 m_bCheckHeader;
	int8 m_delayedWriting;
	int8 m_delayedError;
};
