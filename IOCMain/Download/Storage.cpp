 ...
 * All rights reserved.
 * RCS: $Header: Storage.cpp 1.1 2007/12/06 11:41:21PST Zheng Miao (zmiao) Exp  $
 *	$Revision: 1.1 $
 */
...
//////////////////////////////////////////////////////////////////////
CStorage::CStorage(const char *pName)
{
	strncpy(m_name, pName, 29);
	m_name[29] = 0;
}
...
CCodeStorage::CCodeStorage(const char *pName, CLoader *pLoader, uint32 startAddress /* = MAIN_CODE_START */)
: CStorage(pName)
{
...
int CCodeStorage::CloseStorage(uint32 id)
{
	...
	if (m_user == id) {
		...
		// For TC1000, check this is a different version
		CNewCode::s_pNewCode->Refresh();
...
int CCodeStorage::EndOfDownload(void) 
{
	// CAllProcess::g_pAllProcess->Reboot();
...
// return 1 for OK
// -1 for fail or not supported
int CStorage::Seek(int32 offset)
{
	return -1;
}
